...
CachingProxy.prototype = {
  ...
  storeResponse: function(proxyRequest, upstreamResponse) {
    ...
    if (result = (headers['cache-control'] || '').match(/lifetime=(\d+)/)) {
      lifetime = result[1];
    } else if (result = (headers['x-cache-lifetime'] || '').match(/^(\d+)$/)) {
      lifetime = result[1];
    } else {
      lifetime = 3600; // Default - TODO
    }

    // Remove X-Cache-* headers
    headers = _.pick(headers, function(value, key){  return ! key.match(/^x-cache/);  });
  ...
  checkCacheResult: function(proxyRequest, err, records) {
  ...
  cacheHit: function(proxyRequest, record) {
    this.cache.get(record, this.cacheHitCallback.bind(this, proxyRequest));
  },

  cacheHitCallback: function(proxyRequest, err, record) {
    if (err) {
